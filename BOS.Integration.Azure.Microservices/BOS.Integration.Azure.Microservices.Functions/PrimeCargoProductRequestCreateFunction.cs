    ...
    public class PrimeCargoProductRequestCreateFunction
    {
        ...
        public async Task<Message> Run([ServiceBusTrigger("azure-topic-prime-cargo-wms-product-request", "azure-sub-prime-cargo-product-create-request", Connection = "serviceBus")] string mySbMsg, ILogger log)
        {
            try
            {
                ...
                await this.logService.AddErpMessageAsync(messageObject.ErpInfo, ErpMessageStatus.CreateMessage);
                ...
                await this.logService.AddTimeLineAsync(messageObject.ErpInfo, TimeLineDescription.ProductCreateMessageSentServiceBus, TimeLineStatus.Information);

                // Use prime cargo API to create the object
                var response = await this.primeCargoService.CreateOrUpdatePrimeCargoProductAsync(messageObject.PrimeCargoProduct, ActionType.Create);
                ...
                {
                    string errorMessage = string.IsNullOrEmpty(response.Error) ? "Could not create a new object via prime cargo API" : response.Error;
