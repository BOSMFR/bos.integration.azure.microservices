...
namespace BOS.Integration.Azure.Microservices.Functions.Api
{
    public class GetTimeLinesFunction
    {
            ...
            ILogger log)
        {
            ...
            var timeLineRequest = JsonConvert.DeserializeObject<TimeLineRequestDTO>(requestBody);

            var timeLines = await logService.GetTimeLinesByFilterAsync(timeLineRequest);
